...
//        }
    ...
    private static class Builtin extends Module {
        ...
        public Builtin() {
            ...
            setAttribute("print", new NativeFunction(this, "print", 1, Function.FLAG_VARIADIC, (machine, args) -> {
                machine.getOut().print(Stream.of(((ArrayValue) args[0]).getValues()).map(Object::toString).collect(Collectors.joining(" ")));
                return NoneValue.INSTANCE;
            }));

            setAttribute("println", new NativeFunction(this, "println", 1, Function.FLAG_VARIADIC, (machine, args) -> {
                machine.getOut().println(Stream.of(((ArrayValue) args[0]).getValues()).map(Object::toString).collect(Collectors.joining(" ")));
                return NoneValue.INSTANCE;
            }));

            setAttribute("debug", new NativeFunction(this, "debug", 1, Function.FLAG_VARIADIC, (machine, args) -> {
